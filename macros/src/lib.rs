...
use syn::{DeriveInput, parse_macro_input, Data, Fields, Visibility};
...
fn generate_circuit_impl(input: &DeriveInput) -> syn::Result<TokenStream> {
    ...
    // Generate append_public method implementation
    let append_public_impl = public_fields.iter().map(|(field_name, _)| {
        quote! {
